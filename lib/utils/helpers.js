...
const generateCompilationLog = (startTime, allAssets, generatedFiles, disposedFiles) => {
  ...
  const diffText = diff > oneSecond ?
    ...
    `${diff} ms`;
  const pre = '🏁 ';
  const mainStr = capitalize(main || 'compiled');
  return `${pre} ${mainStr} in ${diffText}`;
};

const animationLogInterval = 300;

const getCompilationProgress = (timePassed, format) => {
  ...
  const lineStart = format('info');

  const writeWithDots = () => {
    const pre = '⚒ ';
    const msg = iterations === 100 ? 'Still compiling' : 'Compiling';
    const end = '.'.repeat(iterations % 4);
    const line = ` ${pre} ${msg}${end}`

    logUpdate.stderr(lineStart + line);
